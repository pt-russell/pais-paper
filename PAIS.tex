...
\begin{abstract}
  ...
  the number of processors used. We also introduce a new resampling
...
and simply implement it simultaneously over a set of processors. All
...
parallelised scenario, each process must perform this process
...
we wish to use to infer $x$. Since we have $M$ processes, we
...
processes in the tail of a Gaussian curve searching for the area of
high density. If the processes are evenly spaced, then the particle
...
\cite{Ji2013adaptive}. 
% Algorithm~\ref{tab:adapt} in Appendix~\ref{Sec:App}, is not presented as an optimal strategy but as a demonstration that the PAIS algorithm can be adaptively tuned effectively.
...
where $x_{i-1}$ is our current state and $y$ is our proposed state. In the numerics section we will refer to $\beta$ as the scaling parameter.
 ...
 algorithms\cite{roberts2007coupling,roberts2009example}, these $n_k$
...
posterior are, is $D_{KL}(\mu_D || \mu_0) = 4.67$ for this problem.
...
\subfigure[Comparing of the relative $L^2$ error of the algorithms optimised by $L^2$ minimisation with locally optimised PAIS-RW with a scout chain.]{\includegraphics[width=0.45\textwidth]{"figures/BM2_scout"}}
...
Comparing the convergence of the adaptive algorithms against the nonadaptive algorithms in Figure~\ref{fig:BM2_AL2} shows that the algorithms behave as expected. The adaptive RWMH algorithm tuned using the acceptance rate converges at the same rate as the $L^2$ optimised algorithm until the scaling parameter gets small where the algorithm forgets one of the modes. The adaptive PAIS-RW with scout chains tuned to the effective sample size converges at about the same rate as the locally optimised nonadaptive algorithm also with scouts.
...
\caption{Illustration of calculating the number of PAIS-pCNL iterations required to reach a tolerance of $10^{-2.5}$ as a percentage of MH iterations. The relative $L^2$ error graphs are from the Gaussian problem in Section~\ref{sec:problem 1}.}
