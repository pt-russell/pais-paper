...
\section{Approximate Multinomial Resampling}
...
the bootstrap resampler, which simply takes the $N$ ensemble members'
weights and constructs a multinomial distribution, from which $N$
...
of the modes again. The bootstrap filter also does not guarantee to
preserve the mean of the weighted sample as the ETPF.

Ideally, we would like to use a resampling algorithm which is not
prohibitively costly for moderately or large sized ensembles, but
which also preserves the mean of the samples, and also makes it much
harder for the new samples to have missed a significant region in the
density. This motivates the following algorithm, which we refer to as
approximate multinomial resampling (AMR).

Instead of sampling $N$ times from an $N$-dimensional multinomial
distribution as is the case with the bootstrap algorithm, we sample
once each from $N$ different multinomials. Suppose that we have $N$
samples $y_n$ with weights $w_n$. The multinomial sampled from in the
bootstrap filter has a vector of probabilities given by:
\begin{equation*}
\frac{1}{\sum w_n} [w_1,w_2,\ldots,w_N] = \bar{\bf w},
\end{equation*}
with associated states $y_n$.
We wish to find $N$ vectors $\{{\bf p}_1,{\bf p}_2,\ldots,{\bf p}_N\}
\subset \mathbb{R}^N_{\ge 0}$
such that  $\frac{1}{N} \sum {\bf p}_i = \bar{\bf w}$. The AMR is then
given by a sample from each of the multinomials defined by the vectors
${\bf p}_i = [p_{i,1},p_{i,2},\ldots,p_{i,N}]$ with associated states ${\bf x}_i$. Alternatively, as with the ETPF, a deterministic sample
...
\begin{equation}
\hat{x}_i = \sum p_{i,j} x_j, \qquad i \in \{1,2,\ldots,N.
\end{equation}
...
bootstrap filter into $N$ pieces, and we can guarantee that exactly
...
to split the original multinomial up into $N$ parts, some of which
...
bootstrap and AMR), we drew a sample of size $N$ from the proposal distribution
...
three moments of the samples changes with $N$ for the three different
...
each region with weights up to $\frac{1}{N}$ of the total.
...
\begin{figure}[htb]
\centering
\subfigure[Relative L2 error.]{\includegraphics[width=0.45\textwidth]{"figures/G2_l2"}}
...
\begin{figure}[htb]
\centering
\subfigure[Relative L2 error.]{\includegraphics[width=0.45\textwidth]{"figures/BM1_L2"}}
...
\begin{figure}[htb]
\centering
\subfigure[Relative L2 error using the optimal scaling parameters.]{\includegraphics[width=0.45\textwidth]{"figures/BM2_L2"}}
\subfigure[Comparing of the relative L2 error of the algorithms optimised by L2 minimisation with locally optimised PAIS-RW with a scout chain.]{\includegraphics[width=0.45\textwidth]{"figures/BM2_scout"}}
...
\subfigure[Relative L2 error against number of iterations.]{\includegraphics[width=0.45\textwidth]{"figures/C1_L2"}}
